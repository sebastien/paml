...
def _guess_source_encodings(data):
    encodings = []
...
def decode_source_bytes(data, fallback="utf-8"):
    for encoding in _guess_source_encodings(data):
        try:
...
def read_source_lines(path):
    with open(path, "rb") as f:
        return decode_source_bytes(f.read()).splitlines(True)


def _run_embedded_command(command, source, path_suffix=""):
    if not command or not shutil.which(command[0]):
        return source
    with tempfile.NamedTemporaryFile("wb", suffix=path_suffix, delete=False) as handle:
        handle.write(ensure_bytes(source))
    ...
    try:
        ...
        return decode_source_bytes(output) if output else source
    finally:
...
class Macro:
    ...
    def ImportJS(parser, params, indent):
        ...
        modules = flatten(
            [
                Macro.Require(_.strip().replace(".", "/"), Macro.JS_PATTERNS)
...
class Element:
    ...
    def isTextOnly(self):
        if len(self.content) == 0:
            ...
            len(self.content) == 1
            and isinstance(self.content[0], Text)
...
class Parser:
    ...
    def __init__(self, formatter=None, defaults=None):
        ...
        self._paths = []
        self._searchPaths = [".", "src/py/paml", "lib/paml"]
...
class HTMLFormatter:
    ...
    def _formatElement(self, element):
        ...
        if mode and mode.startswith("sugar"):
            ...
            res = _run_embedded_command([command] if command else [], source, ".sjs")
            logging.info(
            ...
            source = "".join(lines)
            t = time.time()
            res = _run_embedded_command(["coffee", "-cp"], source, ".coffee")
            logging.info(
            ...
            source = "".join(lines)
            t = time.time()
            res = _run_embedded_command(["tsc"], source, ".ts")
            logging.info(
            ...
            source = "".join(lines)
            t = time.time()
            res = _run_embedded_command(["pcss"], source, ".pcss")
            logging.info(
